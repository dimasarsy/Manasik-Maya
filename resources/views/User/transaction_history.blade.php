        ...
        <div class="container" data-aos="fade-up">
            <div class="section-title">
                ...
                @forelse ($orders as $order)
                    <a href="/detailHistoryTransaction/{{ $order->id }}">
                        <div class="mySchedule-card row mb-5 mx-5 col-gap">
                            ...
                            <div class="col">
                                <div class="row">
                                    <div class="col">
                                        <h3 class="fw-bold" style="color: #87B7AB;">{{ $order->schedule->name }}</h3>
                                    </div>
                                </div>
                                <div class="row mt-3 d-flex" style="color:#B5B5B5; width: 70%;">
                                    <div class="col d-flex gap-3">
                                        <i style="font-size: 20px;" class="icon-calendar-empty"></i>
                                        <p>{{ \Carbon\Carbon::parse($order->schedule->date)->locale('id')->settings(['formatFunction' => 'translatedFormat'])->format('j F Y') }}</p>
                                    </div>
                                    <div class="col d-flex gap-3">
                                ...
                                <div class="row mt-3">
                                    <div class="col">
                                        ...
                                        <p style="color: #373F41;">{{ $order->schedule->description }}</p>
                                    </div>
                                </div>
                                <div class="row mt-3">
                                    <div class="col">
                                        <h5 style="font-weight: bold; color: black">status</h5>
                                        <p style="color: #373F41;">{{ $order->status }}</p>
                                    </div>
                                </div>
                                <div class="row mt-3">
                                    <div class="col">
                                        <h5 style="font-weight: bold; color: black">Masukkan Transaksi Pembayaran Anda</h5>
                                        <p style="color: #373F41;">{{ $order->payment_code }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="text-end" style="font-family: 'Inter'; font-style: normal;">
                                <?php $now = Carbon\Carbon::now()->toDateTimeString(); ?>
                                @if($order->schedule->dueDateSchedule <= $now)
